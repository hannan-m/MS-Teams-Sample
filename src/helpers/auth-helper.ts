        ...
        if (authenticationContext.isCallback(window.location.hash)) {
            ...
            if (window.opener) {
                if (authenticationContext.getCachedUser()) {
                    authenticationContext.acquireToken("https://graph.microsoft.com", (err: string | null, token:string | null) => {
                        if (token) {
                    ...
                    msTeams.authentication.notifyFailure(authenticationContext.getLoginError());
                }
            }
        }
    }
